...
| Robot visibly rises off the floor on every base teleport | `Z_HEIGHT` in `isaac_sim_teleport_listener.py` doesn't match the base's actual resting world Z (it's passed straight into a world-space pose call, so a wrong value moves the robot vertically) | Select `/open_manipulator_x` in the Stage tree and read its Translate Z — `Z_HEIGHT` must equal the base's resting world Z (`-0.09621` here), not `0.0` (BUG-21) |
| A prim's Z in the Property panel disagrees with `Articulation.get_world_pose()` | Not a contradiction — the panel shows **local** transform (relative to parent), the API reports **world** space | Check the parent container's own Translate; here `/open_manipulator_x` carries `-0.09621`, so the child's local `+0.09621` is world `0` |
